...
import static org.firstinspires.ftc.teamcode.lancers.auton.model.AllianceColor.BLUE;

/**
 * Holds common code shared between different auton modes.
 * Implements {@link org.firstinspires.ftc.robotcontroller.external.samples.ConceptDoubleVision} & uses {@link LancersRobot.LancersMecanumDrive}
 */
@Config
public class FullAutonOpMode extends LancersAutonBaseOpMode {
    ...
    private final @NotNull DetectionType detectionType = DetectionType.OPENCV; // tensorflow code works, but its so iffy and we never tested it so lets just rely on our cubes
    private final boolean shouldPlaceYellowPixel;
    private final boolean shouldUseAlternateParkingSpot;
    private final boolean shouldCycle;

    public FullAutonOpMode(final @NotNull StartPosition startPosition, final boolean shouldPlaceYellowPixel, final boolean alternateParkingSpot) {
        super(startPosition);
        this.shouldPlaceYellowPixel = shouldPlaceYellowPixel;
        this.shouldUseAlternateParkingSpot = alternateParkingSpot;
        this.shouldCycle = false; // TODO: implement cycling
    }

    private void initDrive() {
        Objects.requireNonNull(robot).getDrive().setPoseEstimate(startPosition.getStartPose());
    }
    ...
    public @NotNull Pose2d getParkingPosition() {
        if (shouldUseAlternateParkingSpot) {
            switch (startPosition.getStagePosition()) {
                case BACK:
                    return startPosition.getAllianceColor().getFrontstageParkingSpot();
                case FRONT:
                default:
                    return startPosition.getAllianceColor().getBackstageParkingSpot();
            }
        } else {
            switch (startPosition.getStagePosition()) {
                case BACK:
    ...
    public @NotNull TrajectorySequence getTrajectorySequence(final @NotNull TeamScoringElementLocation teamScoringElementLocation) {
        ...
        // we may be placed with an alliance that has a better auton and/or needs to be placed on the backstage, so we can go frontstage and only place yellow
        if (shouldPlaceYellowPixel) {
            // get backstage
            switch (startPosition.getStagePosition()) {
                case BACK:
                    builder.lineToConstantHeading(startPosition.getSafeMovementPose().vec());
        ...
        if (!shouldCycle) {
            // skip straight to parking
            if (shouldPlaceYellowPixel || startPosition.getStagePosition() == StagePosition.BACK) {
                builder.splineTo(getParkingPosition().vec(), getParkingPosition().getHeading());
            }
    ...
    private void doStateSwitchActivity(final @NotNull State newState) {
        ...
        switch (newState) {
            ...
            case TRAJECTORY:
                ...
                break;
            case PARK_AFTER_CYCLE:
                // chopped from other code
                drive.followTrajectorySequenceAsync(drive.trajectorySequenceBuilder(drive.getPoseEstimate())
                        .splineTo(getParkingPosition().vec(), getParkingPosition().getHeading())
                        .build());
                break;
            case DONE:
    ...
    private void runStateMachine() {
        ...
        synchronized (stateMachineLock) {
            switch (state) {
                ...
                case CYCLE:
                    // TODO: cycling code
                    switchStateTo(State.PARK_AFTER_CYCLE);
                    break;
                case PARK_AFTER_CYCLE:
                case TRAJECTORY:
                    if (drive.isBusy()) {
